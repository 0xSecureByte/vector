...
pub fn build_client(
    ...
    match (connection_string, storage_account) {
                ...
                Some(uri) => ClientBuilder::with_location(
                    CloudLocation::Custom {
                        ...
                        account: connection_string
                            .account_name
                            .ok_or("Account name missing in connection string")?.to_string(),
                    },
        ...
        (None, Some(storage_account_p)) => {
            ...
            let storage_credentials = StorageCredentials::token_credential(std::sync::Arc::new(creds));

            client = match endpoint {
                Some(endpoint) => ClientBuilder::with_location(
                    CloudLocation::Custom { 
                        uri: endpoint,
                        account: storage_account_p.clone(),
                    },
                    storage_credentials,
                ),
